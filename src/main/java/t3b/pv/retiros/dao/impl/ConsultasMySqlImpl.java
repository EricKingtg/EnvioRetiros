...
public class ConsultasMySqlImpl implements ConsultasMySql {
    ...
    public List<RetirosDto> obtieneRetiros(int id) {
        log.info("ConsultasMySqlImpl.obtieneRetiros");
        QryRespDTO resp = null;
        ...
        try {
            if (conn != null) {
                ...
                paramsIn.add(id);
                resp = new Consulta().ejecutaSelectSP(conn, IQryRetiros.CONSULTA_INFO_RETIROS, paramsIn);
                if (resp.getRes() == 1) {
                    ArrayList<ColumnaDTO> cols = resp.getColumnas();
                    for (HashMap<String, CampoDTO> fila : resp.getDatosTabla()) {
                        ...
                        for (ColumnaDTO col : cols) {
                            switch (col.getIdTipo()) {
                                ...
                                case java.sql.Types.BIT:
                                    dato.getClass().getField(col.getEtiqueta()).set(dato,
                                            fila.get(col.getEtiqueta()).getValor() == null ? false : Boolean.parseBoolean(fila.get(col.getEtiqueta()).getValor().toString().replaceAll(",", "").replaceAll("$", "").replaceAll(" ", "")));
                                    break;
