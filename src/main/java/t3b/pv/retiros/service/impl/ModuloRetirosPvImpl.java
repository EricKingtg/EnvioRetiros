...
public class ModuloRetirosPvImpl implements ModuloRetirosPv {
	...
	public boolean processRetiro(int idReg) {
		...
		listRetiros = consultas.obtieneRetiros(idReg);
		if (listRetiros.isEmpty()) {
			log.info("No existen registros para ese ID");
			System.exit(0);
		}

		log.info("Paso 4: vamos a insertarlo en SYBASE");

		if (consulSB.insertaRetiros(ipBO, listRetiros.get(0), lista.get(0).getTclave())) {
			log.info("Paso 5: Si se inserta en Sybase Se actualiza en MySql");
			salida = consultas.actualizaRetiro(idReg);
		} else {
